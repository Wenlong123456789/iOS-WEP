...
- (instancetype)init {
  if ((self = [super init])) {
    ...
    self.valid = YES;
    self.session = [VLStringMemorySession new];
...
static void RunTests(void) {
  ...
  VLStringMemorySession *s = f.session;
  ...
  for (VLStringMemoryRecord *r in s.records) byAddress[@(r.address)] = r.text;
  ...
  uint64_t parsed = 0;
  Check([VLStringMemorySession parseAddress:@" 280000000 " value:&parsed] && parsed == 0x280000000ULL, "bare addresses parsed as hexadecimal");
  Check([VLStringMemorySession parseAddress:@"0Xabcdef" value:&parsed] && parsed == 0xabcdef, "hex prefix accepted");
  for (NSString *bad in @[@"", @"0", @"-1", @"+123", @"12xz", @"10000000000000000"])
    Check(![VLStringMemorySession parseAddress:bad value:&parsed], "invalid address rejected");

  NSMutableData *binary = [NSMutableData dataWithLength:256];
  for (NSUInteger i = 0; i < 256; i++) ((uint8_t *)binary.mutableBytes)[i] = i;
  NSString *escaped = [VLStringMemorySession escapedTextForData:binary];
  Check([[VLStringMemorySession dataForEscapedText:escaped] isEqualToData:binary], "all 256 byte values round-trip losslessly");
  NSData *unicode = [@"中😀\\line\n" dataUsingEncoding:NSUTF8StringEncoding];
  Check([[VLStringMemorySession dataForEscapedText:[VLStringMemorySession escapedTextForData:unicode]] isEqualToData:unicode], "Unicode and literal backslashes round-trip");
  for (NSString *bad in @[@"\\", @"\\x", @"\\xQ2", @"\\q"])
    Check([VLStringMemorySession dataForEscapedText:bad] == nil, "malformed escape rejected");
  uint8_t zeros[] = {'a', 0, 'b'};
