...
std::map<std::string, std::string> getLangTW() {
    return {
      ...
      {"Str_Reload_Button", "重載"},
        ...
        {"Str_Write_Unverified", "寫入未能通過回讀驗證，部分位元組可能已變化；原始快照已保留用於復原。請檢查目標狀態。"},
        {"Str_No_Undo", "本次編輯工作階段暫無可復原修改。"},
        {"Str_Reload", "重新載入選取記憶體"},
        {"Str_Load_Before", "前 1 KB"},
        {"Str_Load_After", "後 1 KB"},
        {"Str_Select_Range", "選擇範圍編輯"},
        {"Str_Context", "附近候選字串（點擊切換編輯）"},
        {"Str_Range_Mode", "範圍模式 · 跳脫位元組 · 等長寫入"},
        {"Str_Single_Mode", "單條字串 · 前後文唯讀，點擊切換"},
        {"Str_Escape_Help", "等長替換；\\0 空位元組，\\xNN 十六進位，\\\\ 反斜杠"},
        {"Str_Shorter_Allowed", "允許等長或縮短，自動寫入結束符"},
        {"Str_Count_Fmt", "原始 %lu 位元組 · 修改後 %lu 位元組\n%@"},
        {"Str_Unsaved", "目前內容已修改"},
        {"Str_Discard", "放棄草稿"},
        {"Str_Range_Help", "選擇真實連續的記憶體範圍，最多 8192 位元組。位址按十六進位解析，包含結束位址。分隔符和二進位位元組以跳脫顯示，寫入必須保持位元組總數相等。"},
        {"Str_Start", "起始位址（十六進位）"},
        {"Str_End", "結束位址（包含，十六進位）"},
        {"Str_Saved", "寫入並回讀驗證成功，可繼續編輯或復原。"},
        {"Str_Range_Preview", "0x%llX – 0x%llX\n%lu 位元組，等長替換\n\n原內容：%@\n\n修改後：%@"},
        {"Btn_Hide_Keyboard", "收起鍵盤"},
        {"Btn_Close", "關閉"},
        {"Btn_OK", "確定"},
        {"Btn_Edit", "編輯"},
        {"Btn_Delete", "刪除"},
        ...
        {"Timeline_Restore_Values_Title", "恢復記憶體數值"},
        {"Timeline_Restore_Values_Msg", "同時恢復該搜尋快照記錄的數值？"},
        {"Timeline_Restore_Values_Action", "恢復數值"},
        {"Timeline_Restore_Values_Success_Fmt", "已恢復 %lu 個記憶體數值"},
        ...
        {"Snapshot_Button", "快照"},
        {"Snapshot_Title", "數值快照"},
        {"Snapshot_Save", "儲存目前數值"},
        {"Snapshot_Restore", "恢復快照數值"},
        {"Snapshot_Saved_Fmt", "已儲存 %lu 個數值"},
        {"Snapshot_Restored_Fmt", "已恢復 %lu 個數值"},
        {"Snapshot_Empty", "暫無可讀取數值"},

        // 記憶體搜尋附加
        {"Mem_GroupHint", "使用 ;、*、?、w:8 或 :: 分隔多個值"},
