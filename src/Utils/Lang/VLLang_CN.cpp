...
std::map<std::string, std::string> getLangCN() {
    return {
      ...
      {"Str_Reload_Button", "重载"},
        ...
        {"Str_Write_Unverified", "写入未能通过回读验证，部分字节可能已变化；原始快照已保留用于撤销。请检查目标状态。"},
        {"Str_No_Undo", "本次编辑会话暂无可撤销修改。"},
        {"Str_Reload", "重新加载选中内存"},
        {"Str_Load_Before", "前 1 KB"},
        {"Str_Load_After", "后 1 KB"},
        {"Str_Select_Range", "选择范围编辑"},
        {"Str_Context", "附近候选字符串（点击切换编辑）"},
        {"Str_Range_Mode", "范围模式 · 转义字节 · 等长写入"},
        {"Str_Single_Mode", "单条字符串 · 前后文只读，点击切换"},
        {"Str_Escape_Help", "等长替换；\\0 空字节，\\xNN 十六进制，\\\\ 反斜杠"},
        {"Str_Shorter_Allowed", "允许等长或缩短，自动写入结束符"},
        {"Str_Count_Fmt", "原始 %lu 字节 · 修改后 %lu 字节\n%@"},
        {"Str_Unsaved", "当前内容已修改"},
        {"Str_Discard", "放弃草稿"},
        {"Str_Range_Help", "选择真实连续的内存范围，最多 8192 字节。地址按十六进制解析，包含结束地址。分隔符和二进制字节以转义显示，写入必须保持字节总数相等。"},
        {"Str_Start", "起始地址（十六进制）"},
        {"Str_End", "结束地址（包含，十六进制）"},
        {"Str_Saved", "写入并回读验证成功，可继续编辑或撤销。"},
        {"Str_Range_Preview", "0x%llX – 0x%llX\n%lu 字节，等长替换\n\n原内容：%@\n\n修改后：%@"},
        {"Btn_Hide_Keyboard", "收起键盘"},
        {"Btn_Close", "关闭"},
        {"Btn_OK", "确定"},
        {"Btn_Edit", "编辑"},
        {"Btn_Delete", "删除"},
        ...
        {"Timeline_Restore_Values_Title", "恢复内存数值"},
        {"Timeline_Restore_Values_Msg", "同时恢复该搜索快照记录的数值？"},
        {"Timeline_Restore_Values_Action", "恢复数值"},
        {"Timeline_Restore_Values_Success_Fmt", "已恢复 %lu 个内存数值"},
        ...
        {"Snapshot_Button", "快照"},
        {"Snapshot_Title", "数值快照"},
        {"Snapshot_Save", "保存当前数值"},
        {"Snapshot_Restore", "恢复快照数值"},
        {"Snapshot_Saved_Fmt", "已保存 %lu 个数值"},
        {"Snapshot_Restored_Fmt", "已恢复 %lu 个数值"},
        {"Snapshot_Empty", "暂无可读取数值"},

        // 内存搜索附加
        {"Mem_GroupHint", "使用 ;、*、?、w:8 或 :: 分隔多个值"},
