...
#define STRING_TEXT(key) VL(key)

@interface VLStringEditorViewController () <UITextViewDelegate, UITableViewDataSource, UITableViewDelegate>
...
@property(nonatomic, copy) NSArray<VLStringMemoryRecord *> *contextRecords;
...
@implementation VLStringEditorViewController
...
- (void)viewDidLoad {
  ...
  self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]
      initWithTitle:STRING_TEXT(@"Alert_Cancel") style:UIBarButtonItemStylePlain
      target:self action:@selector(cancelEditing)];
  self.saveButton = [[UIBarButtonItem alloc]
      initWithTitle:STRING_TEXT(@"Edit_Save") style:UIBarButtonItemStyleDone
...
- (void)scrollToSelection {
  ...
  for (NSUInteger i = 0; i < self.contextRecords.count; i++) {
    VLStringMemoryRecord *record = self.contextRecords[i];
...
- (void)updateCount {
  VLStringMemorySession *s = self.session;
  ...
  NSData *bytes = s.rangeMode ? [VLStringMemorySession dataForEscapedText:self.textView.text] :
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)path {
  ...
  VLStringMemoryRecord *record = self.contextRecords[path.row];
...
- (void)loadContextBefore:(BOOL)before {
  ...
  VLStringMemoryRecord *anchor = first && first.row < self.contextRecords.count ? self.contextRecords[first.row] : nil;
...
- (void)resolveDraft:(void (^)(void))action {
  ...
  [alert addAction:[UIAlertAction actionWithTitle:STRING_TEXT(@"Alert_Cancel") style:UIAlertActionStyleCancel handler:nil]];
  ...
  [alert addAction:[UIAlertAction actionWithTitle:STRING_TEXT(@"Edit_Save") style:UIAlertActionStyleDefault
      handler:^(UIAlertAction *a) {
  ...
  [self resolveDraft:^{
    ...
    [alert addAction:[UIAlertAction actionWithTitle:STRING_TEXT(@"Btn_OK") style:UIAlertActionStyleDefault
        handler:^(UIAlertAction *a) {
          ...
          NSString *error = nil;
          if (![VLStringMemorySession parseAddress:alert.textFields[0].text value:&start] ||
              ![VLStringMemorySession parseAddress:alert.textFields[1].text value:&end]) error = @"Str_Invalid_Range";
...
- (void)saveWithCompletion:(void (^)(void))completion {
  ...
  [alert addAction:[UIAlertAction actionWithTitle:STRING_TEXT(@"Alert_Cancel") style:UIAlertActionStyleCancel handler:nil]];
  [alert addAction:[UIAlertAction actionWithTitle:STRING_TEXT(@"Edit_Save") style:UIAlertActionStyleDestructive
      handler:^(UIAlertAction *a) {
